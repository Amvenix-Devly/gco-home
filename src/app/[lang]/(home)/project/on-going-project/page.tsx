...
const OnGoingProject = async () => {
  const allOnGoingProject = await db.project.findMany({
    where: {
      projectType: ProjectType.ON_GOING_PROJECT,
    },
    orderBy: {
      startTime: 'asc',
    },
  ...
  return (
    <main className="container mb-20">
      <h1 className="text-center text-4xl my-10">On Going Projects</h1>
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {allOnGoingProject.map((project, index) => (
